...
func (r *Recursor) verifyDS(ctx context.Context, q dns.Question, isIPV6 bool) bool {
	...
	for {
		...
		if name == "." {
			...
			resp, _, err := r.resolveNS(ctx, dsreq.Question[0], isIPV6, 0, nil)
			if err != nil {
				return false
			}
...
// error
func (r *Recursor) resolveNSOld(ctx context.Context, req *dns.Msg, isIPV6 bool, offset int) (*dns.Msg, *authServers, error) {
	q := req.Question[0]
	...
	resp, rservers, err := r.resolveNS(ctx, q, isIPV6, i, nil)
	if err != nil {
		return resp, nil, err
	}
	...
	servers, _, err := r.buildServers(ctx, resp, zone, isIPV6)
	// if servers == nil && strings.HasSuffix(q.Name, "arpa.") {
	// 	log.Print("======================================")
	// 	log.Print(toResolve)
	// 	s := &authServers{
	// 		Zone: zone,
	// 	}
	// 	r.resolveExtraNs(ctx, toResolve, zone, s, isIPV6)
	// 	s.RLock()
	// 	if len(s.List) != 0 {
	// 		err = nil
	// 		servers = s
...
func (r *Recursor) resolveNS(ctx context.Context, q dns.Question, isIPV6 bool, depth int, servers *authServers) (*dns.Msg, *authServers, error) {
	// ALGO:
	// starts from root
	// dig . ns
	// dig com ns
	// dig google.com ns
	// dig api.google.com ns
	// STOP: no more ns records in response
	if servers == nil {
	...
	if err == nil {
		...
		_, nextServers, _ := r.resolveNS(ctx, q, isIPV6, depth+1, cached)
	...
	nextServers, toResolve, err = r.buildServers(ctx, resp, zone, isIPV6)
	if err != nil {
		// if err == errNoNSfound && len(toResolve) == 0 {
		// log.Print(resp)
		soa := r.findSoa(resp)
		if soa != nil {
	...
	resp, nextServers, err = r.resolveNS(ctx, q, isIPV6, depth+1, nextServers)
...
func (r *Recursor) getDNSKEY(ctx context.Context, zone string, isIPV6 bool) []dns.RR {
	...
	_, rservers, err := r.resolveNS(ctx, q, isIPV6, 0, nil)
	if err != nil {
		return keys
	}
...
func (r *Recursor) resolve(ctx context.Context, req *dns.Msg, isIPV6 bool) (*dns.Msg, error) {
	...
	// query upper servers if we are asking for DS
	query := dns.Question{
		Qtype:  q.Qtype,
		Qclass: q.Qclass,
		Name:   q.Name,
	...
	if q.Qtype == dns.TypeDS {
		idx, end := dns.NextLabel(q.Name, 0)
		name := "."
		if !end {
			name = q.Name[idx:]
	...
	nsResp, servers, err := r.resolveNS(ctx, query, isIPV6, 0, nil)
	if err != nil {
	...
	if len(ans.Answer) == 0 && len(ans.Ns) > 0 {
		...
		if nextServers == nil && err == errNoNSfound {
			nextServers = &authServers{
				Zone: q.Name,
			}

			r.resolveExtraNs(ctx, toResolve, q.Name, nextServers, isIPV6)
			nextServers.RLock()
